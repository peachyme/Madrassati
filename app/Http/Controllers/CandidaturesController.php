...
use App\Responsable;
use App\session;
use App\Mail\ProgrammerEntretien;
use App\Mail\AttenteCandidature;
use App\Mail\AccepterCandidature;
use App\Mail\RefuserCandidature;
use Illuminate\Support\Facades\Mail;
use Carbon\Carbon;
use Illuminate\Routing\Route;

class CandidaturesController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function accepter($id)
    {
        ...
        Mail::to($candidature->candidat->email)->send(new AccepterCandidature($info));
        
        return redirect()->route('voyager.candidatures.show' , [$id])->with('status','candidature accepter avec succes');
    }
    public function attente($id)
    {
        $candidature = Candidature::where('id', $id)->first();
        $candidature->etat="attente";
        $candidature->save();
        $info=[
            'nom' => $candidature->candidat->nom,
            'prenom'=> $candidature->candidat->prenom
        ];
        Mail::to($candidature->candidat->email)->send(new AttenteCandidature($info));
        
        return redirect()->route('voyager.candidatures.show' , [$id])->with('status','candidature met en attente avec succes');
    }
    public function valider($id,Request $request)
    {
        ...
        $entretien = new Entretien(['date'=>$request->date , 'lieu'=>$request->lieu , 'heure'=>$request->heure ]);
        $candidature->entretien()->save($entretien);
        ...
        Mail::to($candidature->candidat->email)->send(new ProgrammerEntretien($info));

        $candidature->etat="valider";
        $candidature->save();
        //dd($info);
        return redirect()->route('voyager.candidatures.show' , [$id])->with('status','Entretien enregister avec succes');
    }
    public function refuser($id,Request $request)
    {
        $candidature = Candidature::where('id', $id)->first();
        $candidature->etat="refuser";
        $candidature->motif_refus= $request->motif;
        $candidature->save();
        $info=[
            'motif' => $request->motif,
            'nom' => $candidature->candidat->nom,
        ...
        Mail::to($candidature->candidat->email)->send(new RefuserCandidature($info));
        return redirect()->route('voyager.candidatures.show' , [$id])->with('status','Motif enregister avec succes');
    }
    ...
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
           ...
           $idR = Responsable::where('user_id' , auth()->user()->id  )->value('id');
            $idF = Formation::where('Responsable_id' , $idR )->value('id');
            $idS = session::where('formation_id', $idF)->get();
            //dd(count($idS));
            $array = [];
            ...
            //dd($array);
            $query = $query->whereIN('session_id', $array);
            //dd($query);
            if(request()->path() == 'admin/candidatures_recues')
            $query = $query->where('etat' , 'reçue');
            if(request()->path() == 'admin/candidatures_attentes')
            $query = $query->where('etat' , 'attente');
            if(request()->path() == 'admin/candidatures_validees')
            $query = $query->where('etat' , 'valider');
            if(request()->path() == 'admin/candidatures_refusees')
            $query = $query->where('etat' , 'refuser');
            if(request()->path() == 'admin/candidatures_acceptees')
            $query = $query->where('etat' , 'accepter');
       ...
       /*for ($i = 0; $i < count($dataTypeContent) ; $i++)
        {
            $Sid = $dataTypeContent[$i]->session_id;
            $Fid = session::where('id', $Sid)->value('formation_id');
            $Rid = Formation::where('id' , $Fid)->value('responsable_id');
            $Uid = Responsable::where('id', $Rid)->value('user_id');
            if ($Uid != auth()->user()->id)
            {
    ...
    public function show(Request $request, $id)
    {
        ...
        /*$formation = Formation::where('id',$dataTypeContent->formation_id )->first();
        $candidat = Candidat::where('id',$dataTypeContent->candidat_id )->first();*/
